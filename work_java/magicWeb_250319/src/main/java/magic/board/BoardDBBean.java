...
//		String sql = "select b_id, b_name, b_email, b_title, b_content, b_date, b_hit, b_pwd, b_ip "
		String sql = "select b_id, b_name, b_email, b_title, b_content, b_date, b_hit, b_pwd, b_ip"
				+ ", b_ref, b_step, b_level, b_fname, b_fsize , b_commentcount, b_commentcontent"
...
//					쿼리 결과를 BoardBean 객체에 담아서 ArrayList 에 저장
					...
					board.setB_commentcount(rs.getInt(15));
					board.setB_commentcontent(rs.getString(16));
//					여기까지가 1행을 가져와서 저장
...
//			첨부파일이 있으면
	...
	public int insertcomment(BoardBean board) {
		...
		try {
			...
			sql = "insert into boardt(b_id, b_ref, b_commentcontent) values(?,?,?)";
//			sql = "update boardt set b_commentcount=bIdeConnections%2523local-scott_commentcount+1, b_commentcontent=? where b_id=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, number);
			pstmt.setInt(2, board.getB_ref());
			pstmt.setString(3, board.getB_commentcontent());
			re = pstmt.executeUpdate(); //성공시 1 				
